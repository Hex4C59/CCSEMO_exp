...
def aggregate_folds(base_dir):
    """聚合所有fold的结果并计算统计量"""
    base_path = Path(base_dir)
    
    # 查找所有fold的best_results.txt
    fold_results = []
    for fold_dir in sorted(base_path.glob('*fold*')):
    ...
    for metric in metrics:
        ...
        if values:
            aggregated[metric] = {
                'mean': np.mean(values),
                'std': np.std(values, ddof=0),  # 使用样本标准差
                'values': values
            }
...
def format_results(model_name, aggregated, fold_results):
    """格式化输出结果"""
    lines = []
    lines.append("=" * 80)
...
def main():
    ...
    parser.add_argument('--exp_dir', type=str, required=True, 
                        help='Base directory containing fold results')
    ...
    # 聚合结果
    aggregated, fold_results = aggregate_folds(base_dir)
    
    # 格式化输出
    formatted = format_results(model_name, aggregated, fold_results)
